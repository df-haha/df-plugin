...
> **Orca ADE 使用者注意（opt-in，預設不影響任何流程）**：reviewer 呼叫**預設一律走
> shell headless（runner／CLI 一次性執行）**，在 Orca 內也一樣。只有當使用者**本輪明確
> 要求**「開 pane」「pane 模式」跑 reviewer、且本機存在 `~/.claude/refs/orca-codex-pane.md`
> （pane 工作流的 canonical reference，含腳本與收工紀律）時，才依該檔改走 pane 模式。
> 沒有該檔＝本機未配置 pane 工作流，一律走下方原流程。
