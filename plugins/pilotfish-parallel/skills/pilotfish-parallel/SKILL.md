...
6. Resolve the manifest to an absolute path, then run `python3 "$CODEX_HOME/skills/pilotfish-parallel/scripts/runner.py" validate --manifest "$MANIFEST_ABS"`. The format is defined by [the job schema](schemas/job.schema.json), and role routing is fixed by [the role policy](config/roles.toml).
7. Only after validation succeeds, run the same [runner](scripts/runner.py) with `run --manifest "$MANIFEST_ABS"`.
