"""通用 plugin 碼內不得有 tenant hard-code 或密鑰形狀（om-daily-work-log 版）。

om-daily-work-log 的 scripts/skills 是 department-agnostic 的——員工只裝這一個 plugin，
所有 tenant 特定值（member_id、主管 email、clone 路徑）都走 config / 佔位符，碼內不寫死。

兩道檢查：
1. test_no_tenant_hardcode — 掃 tenant 識別詞（公司/人名/基礎設施/私有路徑前綴）。
2. test_no_secret_shapes  — 用 regex 掃密鑰「形狀」（不嵌真密鑰字面值）。

掃描範圍刻意排除 tests/（本檔即含禁用詞清單）。
"""
...
EXACT_BANNED = [
    ...
    "-home-haha",        # 私有 home 目錄前綴（去寫死後不該再出現）
]
...
def test_no_tenant_hardcode():
    """通用碼/模板/skill 內不得出現 tenant 識別詞（應移到 config 驅動或佔位符）。"""
    hits = []
...
def test_find_tenant_terms_self_check():
    ...
    assert "-home-haha" in _find_tenant_terms("-home-haha-CC-project-X projects dir")
    # 乾淨字串不誤判（本 plugin 自身名稱不在禁用詞內）
    assert _find_tenant_terms("om-daily-work-log generic code") == []
