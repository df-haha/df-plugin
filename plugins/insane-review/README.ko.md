...
GPT Pro는 ChatGPT 웹 구독에서만 쓸 수 있고 **공식 API가 없다.** 기존 review 흐름은 **로그인된 ChatGPT 웹 세션을 CDP로 자동화**한다. 이 fork의 Deep Research 흐름에서는 Codex CLI와 Claude Code가 전용 CDP 브라우저를 공유하고, ChatGPT desktop Codex chat에서는 공식 Chrome Extension bridge를 선택적으로 사용할 수 있다. 둘 다 기존 ChatGPT 요금제로 동작하며 API 비용은 없다.
...
| `/insane-research [리서치 요청]` | GPT-5.6 Sol／Extra High ChatGPT Deep Research를 실행하고 출처 포함 보고서를 저장 |
| 자연어 | "Pro한테 리뷰시켜줘", "GPT Pro 의견 받아와" — 동일 흐름 |

Codex CLI와 Claude Code는 격리된 CDP 포트 `9333`을 사용한다. Browser／Chrome Extension은 Codex CLI가 아니라 ChatGPT desktop Codex chat의 선택 경로다. 두 경로 모두 모델, 추론 강도, Deep Research 모드, 대화 URL, 완료 상태를 명시적으로 검증한다.
