---
name: df-graph-setup
description: 安裝並登入 df-graph（Microsoft 365 / Graph MCP server）——確認 uv、以 user-scope 註冊 MCP、device-code 登入、selftest 驗證、提醒重啟。每人跑一次。觸發詞：「df-graph setup」「設定 df-graph」「裝 Graph MCP」「df-graph onboarding」。
...
不需要 Windows / Outlook Desktop。全程一次 device-code 登入，token 存本機 `~/.df-graph/`。
...
2. 確認 plugin 已啟用——本 skill 來自 df-graph plugin，`${CLAUDE_PLUGIN_ROOT}` 會指向 vendored `server/`。
...
提醒使用者：**重啟 Claude Code session**，重啟後 `mcp__df-graph__*` 工具才會出現。
驗證：
