...
| **ruling-ledger** | 裁決帳本 — 每個 repo 一本帳本記錄立場裁決（採用／打掉／待裁）。索引 `docs/裁決帳本.md` 永遠載入、詳情 `docs/裁決/R-NNNN.md` 碰關鍵字才讀；`/ledger check` 掃白名單檔與草稿（含 pptx/docx）抓「被打掉的概念復活」，累計復活自動提議升層進硬禁區。Claude 只提議、使用者確認才寫；立場句不進 CLAUDE.md。與 **decision-wiki** 互補：decision-wiki 記「採用」的架構決策，ruling-ledger 記「打掉」的立場並防復活。改編自 Sophia 的 sophia-plugins。 |
